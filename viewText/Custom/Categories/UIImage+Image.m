...
+ (UIImage *)sd_animatedGIFNamed:(NSString *)name {
    ...
    if (scale > 1.0f) {
        ...
        if (data) {
            return [UIImage sd_animatedGIFWithData1:data];
        }
...
+ (UIImage *)sd_animatedGIFWithData1:(NSData *)data {
    if (!data) {
        return nil;
    }
    ...
    if (count <= 1) {
        ...
        for (size_t i = 0; i < count; i++) {
            ...
            duration += [self sd_frameDurationAtIndex1:i source:source];
...
+ (float)sd_frameDurationAtIndex1:(NSUInteger)index source:(CGImageSourceRef)source {
